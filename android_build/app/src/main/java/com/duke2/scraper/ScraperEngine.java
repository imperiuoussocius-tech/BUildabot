...
public class ScraperEngine {
    ...
    private OkHttpClient createAdvancedHttpClient() {
        ...
        builder.addNetworkInterceptor(chain -> {
            ...
            if (response.code() == 403 && bypassMode.equals("cf")) {
                callback.onProgress("⚠️ Cloudflare detected - attempting bypass...");
                // Retry with delay for CF to process
                try {
                    Thread.sleep(1000);
    ...
    public void scrape(String startUrl, int maxDepth, int maxFiles) {
        new Thread(() -> {
            try {
                isRunning = true;
                callback.onProgress("🌐 Starting scraper (" + bypassMode + " mode)...");
                callback.onProgress("🔧 Bypass: " + getBypassDescription());
                ...
                callback.onProgress("📊 Found " + downloadedMedia.size() + " media files");
    ...
    private String getBypassDescription() {
        switch (bypassMode) {
            case "cf":
                return "Cloudflare (header rotation + retry)";
            case "aggressive":
                return "Aggressive (multiple retries)";
            case "stealth":
                return "Stealth (User-Agent rotation + delays)";
            default:
                return "Standard (basic requests)";
        }
    }
    ...
    private void scrapeUrl(String url, int depth, int maxFiles) {
        ...
        try {
            callback.onProgress("🕷️ Crawling: " + url + " (depth: " + depth + ")");
            ...
            callback.onProgress("⚠️ Error scraping " + url + ": " + e.getMessage());
        }
    }
    ...
    private Document fetchDocument(String url) {
        try {
            ...
            // Retry on Cloudflare 403
            if (response.code() == 403 && (bypassMode.equals("cf") || bypassMode.equals("aggressive"))) {
                response.close();
                try {
                    Thread.sleep(2000);
                } catch (InterruptedException e) {
            ...
            callback.onProgress("⚠️ Failed to fetch: " + url);
        }
        return null;
    }
        ...
        new Thread(() -> {
            try {
                ...
                if (file.exists() && file.length() > 0) {
                    ...
                    callback.onProgress("✓ " + filename + " (" + (bytes.length / 1024) + " KB)");
                }
            } catch (Exception e) {
                callback.onProgress("✗ Failed: " + mediaUrl);
            }
        }).start();
    }
    ...
    private String generateGallery() {
        try {
            ...
            html.append(".bypass { background: #1a4d1a; padding: 10px; margin-bottom: 20px; border-radius: 5px; color: #00ff00; }\n");
            ...
            html.append("<div class='bypass'>🛡️ Bypass Mode: ").append(getBypassDescription()).append("</div>\n");
